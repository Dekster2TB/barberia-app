...
import { FaCut, FaUserCog, FaSignOutAlt, FaCalendarCheck, FaLock, FaLaptopCode } from 'react-icons/fa';
...
const Layout = ({ children }) => {
    ...
    if (user) {
        if (user.role === 'developer') {
            panelUrl = '/dev-dashboard';
            panelLabel = 'PANEL MAESTRO';
        ...
        <div className="d-flex flex-column min-vh-100 position-relative overflow-hidden" style={{ backgroundColor: '#f0f2f5' }}>
            ...
            <nav className="navbar navbar-expand-lg navbar-dark bg-primary sticky-top border-bottom border-dark" style={{ zIndex: 10 }}>
                <div className="container">
                    <Link className="navbar-brand d-flex align-items-center fw-bold" to="/" onClick={closeNav} style={{ letterSpacing: '1px' }}>
                        {config.logoUrl ? (
                            <img 
                                src={config.logoUrl} 
                                alt={config.appName} 
                                // AQUI SE APLICA EL CAMBIO DE TAMAÑO EN EL NAVBAR
                                style={{ maxHeight: `${config.logoHeight || 75}px`, objectFit: 'contain', transition: 'max-height 0.3s' }} 
                            />
                        ) : (
                    ...
                    <button className="navbar-toggler" type="button" onClick={toggleNav} aria-expanded={isNavOpen} aria-label="Toggle navigation">
                        ...
                        <ul className="navbar-nav ms-auto align-items-center">
                            ...
                            {panelUrl && (
                                <li className="nav-item ms-lg-2">
                                    <Link className={`nav-link btn btn-outline-light border-0 fw-bold ${isActive(panelUrl)}`} to={panelUrl} onClick={closeNav} style={{ color: '#fbbf24' }}>
                                        <FaLaptopCode className="me-1 mb-1" /> {panelLabel}
                                    </Link>
                                </li>
                            )}
                            ...
                            {user && (
                                <li className="nav-item dropdown ms-lg-3">
                                    <button className="nav-link dropdown-toggle btn btn-secondary text-white px-3 rounded-pill border-0" type="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        <FaUserCog className="me-2" /> {user.username}
                                    </button>
                                    <div className="dropdown-menu dropdown-menu-end shadow border-0 mt-2">
                                        {panelUrl && (
                                            <Link className="dropdown-item d-flex align-items-center" to={panelUrl} onClick={closeNav}>
                                                <FaCalendarCheck className="me-2 text-info" /> Ir al Panel
                                            </Link>
                                        )}
            ...
            <main className="flex-grow-1 d-flex align-items-center py-5 position-relative" style={{ zIndex: 5 }}>
                <div className="container animate__animated animate__fadeIn">
                    <div className={`p-4 p-md-5 rounded-4 shadow-lg ${!hasBackground ? 'bg-white' : ''}`} style={glassStyle}>
                        {children}
                    </div>
                </div>
            </main>
                ...
                <div className="container position-relative">
                    ...
                    {!user && (
                        <div className="position-absolute top-50 end-0 translate-middle-y p-2">
                            <Link to="/login" className="text-secondary opacity-50 hover-opacity-100" title="Acceso Staff"><FaLock size={14} /></Link>
                        </div>
                    )}
                </div>
            </footer>
