...
const BarbersManager = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Petición al backend para crear
    ...
    const handleDelete = async (id) => {
        if(!window.confirm('¿Estás seguro de eliminar a este barbero? Sus citas futuras podrían verse afectadas.')) return;
        ...
        <div className="animate__animated animate__fadeIn">
            ...
            <div className="card shadow-sm mb-5 border-0">
                <div className="card-header bg-dark text-white">
                    <h5 className="mb-0">💈 Contratar Nuevo Barbero</h5>
                </div>
                ...
                {barbers.map(b => (
                    <div key={b.id} className="col-md-3 col-sm-6 mb-4">
                        <div className="card h-100 shadow-sm border-0 text-center p-3">
                            ...
                            <button className="btn btn-outline-danger btn-sm w-100 mt-2" onClick={() => handleDelete(b.id)}>
                                Despedir
                            </button>
                        </div>
                    </div>
